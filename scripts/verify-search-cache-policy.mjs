...
assert.ok(service.includes("cacheKey('search-v7-resilient'"), 'search cache namespace was not invalidated');
assert.ok(!service.includes("cacheKey('search-v6-dual'"), 'legacy search cache namespace still active');
...
assert.ok(router.includes('const upstreamUnavailable = result.total === 0 && Boolean(result.partial);'), 'upstream failure classifier missing');
...
console.log('CACHE_POISON_REGRESSION_OK: partial/empty responses bypass cache; valid results cache; v7 namespace active; Vagalume index-first; 503 retry contract active');
