...
if (!searchRewrite) failures.push('Missing centralized /api/proxy/lyrics/search rewrite');
else if (!String(searchRewrite.destination || '').startsWith('/api/index?__glx_path=')) failures.push('Search rewrite must target /api/index with __glx_path');
...
const indexText = fs.readFileSync(path.join(root, 'api/index.js'), 'utf8');
for (const required of ['proxy_edge_entry', 'adapterPromise', 'logEdgeEntry']) {
  if (!indexText.includes(required)) failures.push(`Missing early-entry/boot resilience in api/index.js: ${required}`);
}
...
console.log(`VERCEL_COMPAT_FUNCTIONS_OK: ${expectedApi.length} API handlers; /search centralizado em /api/index; log de entrada precoce ativo; contrato interativo ativo`);
