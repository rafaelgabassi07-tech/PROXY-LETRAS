...
export { EXTRACTION_ENGINE_NAME, EXTRACTION_ENGINE_VERSION } from './meta.js';
...
export function normalizeLyricsText(value) {
...
function qualityFor(text, linkTextLength = 0) {
    ...
    if (averageLineLength >= 8 && averageLineLength <= 90)
        score += 8;
    ...
    if (duplicateLineRatio > 0.82 && distinctLineCount <= 4 && lines.length >= 12)
        score -= 18;
    else if (duplicateLineRatio > 0.7 && distinctLineCount <= 7 && lines.length >= 16)
        score -= 7;
    ...
    const confidence = clamp(0.08 + (score / 100) * 0.78 + Math.min(0.12, lines.length / 250), 0, 0.99);
    return {
...
function createCandidate(input) {
    ...
    if (text.length < 45 || metrics.words.length < 10 || metrics.lines.length < 2)
        return null;
    ...
    if (text.length >= 250 && text.length <= 20_000)
        score += 6;
    if (metrics.lines.length >= 8 && metrics.lines.length <= 180)
        score += 8;
    if (quality.linkDensity > 0.28)
        score -= 18;
...
function elementText($, element) {
    ...
    clone.find(BLOCK_BREAK_SELECTOR).each((_index, node) => {
        if (node.type === 'tag' && node.name !== 'br')
            $(node).append('\n');
    });
    const linkTextLength = clone.find('a').toArray().reduce((sum, node) => sum + normalizeLyricsText($(node).text()).length, 0);
    return { text: normalizeLyricsText(clone.text()), linkTextLength };
}
function collectDomCandidates(html, parser, add) {
    const $ = parser === 'parse5'
        ? load(html, { scriptingEnabled: false })
        : load(html, { xml: { xmlMode: false, decodeEntities: true } });
    for (const selector of SEMANTIC_SELECTORS) {
        $(selector).slice(0, 20).each((_index, element) => {
    ...
    let inspected = 0;
    let densityAdded = 0;
    $(GENERIC_BLOCK_SELECTORS.join(',')).each((_index, element) => {
        if (inspected >= MAX_DOM_BLOCKS || densityAdded >= 180)
            return false;
        inspected += 1;
        const extracted = elementText($, element);
        const text = extracted.text;
        if (text.length < 100 || text.length > MAX_LYRICS_CHARS)
            return undefined;
        const lineCount = text.split('\n').map(line => line.trim()).filter(Boolean).length;
        const linkDensity = extracted.linkTextLength / Math.max(1, text.length);
        if (lineCount < 3 || linkDensity > 0.42)
            return undefined;
        ...
        add({
            ...
            method: 'dom-density',
            parser,
            source: `density:${String(element.name || 'node')}`,
            linkTextLength: extracted.linkTextLength,
            structuralBonus,
        });
        densityAdded += 1;
...
function parserDom(html, parser) {
    return parser === 'parse5'
        ? load(html, { scriptingEnabled: false })
        : load(html, { xml: { xmlMode: false, decodeEntities: true } });
}
function punctuationWeight(text) {
...
function blockLyricScore(text, linkDensity) {
    const metrics = lineMetrics(text);
    if (metrics.words.length < 4 || text.length < 20)
        return -20;
    ...
    score -= noiseCount * 12;
    if (metrics.averageLineLength > 150)
        score -= 7;
    return score;
}
/**
 ...
 */
function collectReadabilityCandidates(html, parser, add) {
    const $ = parserDom(html, parser);
    const nodeScores = new Map();
    const addScore = (node, amount) => {
        if (!node || node.type !== 'tag' || !Number.isFinite(amount))
            return;
        nodeScores.set(node, (nodeScores.get(node) || 0) + amount);
    };
    let inspected = 0;
    $('p,pre,blockquote,li,div').each((_index, element) => {
        if (inspected >= MAX_DOM_BLOCKS)
            return false;
        inspected += 1;
        const extracted = elementText($, element);
        const text = extracted.text;
    ...
    for (const [topNode, topScore] of ranked) {
        ...
        if (siblings.length < 2 || siblings.length > 120)
            continue;
 ...
 */
function collectBlockClusterCandidates(html, parser, add) {
    const $ = parserDom(html, parser);
    let containers = 0;
    let clusterAdded = 0;
    $('main,article,section,[role="main"],body,div').each((_index, container) => {
        if (containers >= 900 || clusterAdded >= 180)
            return false;
        containers += 1;
        ...
        let run = [];
        const flush = () => {
            if (run.filter(block => block.strong).length >= 2)
                runs.push(run);
            run = [];
        };
 ...
 */
function collectBaselineRescue(html, add) {
    let $;
    try {
        $ = load(html, { scriptingEnabled: false });
    }
    ...
    for (const selector of ['main', 'article', '[role="main"]', 'body']) {
        $(selector).slice(0, 4).each((_index, element) => {
            const clone = $(element).clone();
            clone.find(NOISE_SELECTOR).remove();
            const chunks = [];
            let linkTextLength = 0;
            clone.find('p,pre,blockquote,li,div').slice(0, 900).each((_i, node) => {
                const text = normalizeLyricsText($(node).text());
                if (text.length < 8 || text.length > 2500)
                    return undefined;
                const links = $(node).find('a').toArray().reduce((sum, link) => sum + normalizeLyricsText($(link).text()).length, 0);
                const density = links / Math.max(1, text.length);
                if (density > 0.24 || NOISE_PATTERNS.some(pattern => pattern.test(text)))
                    return undefined;
                chunks.push(text);
...
function looksLikeLyricsValue(value) {
    const normalized = normalizeLyricsText(value);
    if (normalized.length < 45 || normalized.length > MAX_LYRICS_CHARS)
        return false;
    const metrics = lineMetrics(normalized);
    return metrics.lines.length >= 2 && metrics.words.length >= 10;
}
function visitStructuredNode(node, add, method, parser, path = '$', depth = 0, seen = new Set()) {
...
function collectStructuredCandidates(html, add) {
    let $;
    try {
        $ = load(html, { scriptingEnabled: false });
    }
    ...
    $('script').slice(0, 120).each((_index, element) => {
        ...
        if (!isLdJson && !isHydration && !/^\s*[\[{]/.test(raw))
            return;
    ...
    const scanQuotedProperties = (source, parser, maxMatches = 40) => {
    ...
    $('script').slice(0, 160).each((_index, element) => {
        const raw = $(element).text();
        if (!raw || raw.length > 1_500_000 || !/__next_f|react\.server|flight/i.test(raw))
            return undefined;
    ...
    $('noscript,template').slice(0, 60).each((_index, element) => {
        const fragment = $(element).html() || $(element).text() || '';
        if (!fragment || fragment.length > 500_000)
            return undefined;
        const text = htmlFragmentToText(fragment);
        if (looksLikeLyricsValue(text)) {
            add({ rawText: text, method: 'json-embedded', parser: String(element.name || 'template'), source: 'alternate-markup', structuralBonus: 3 });
        }
        return undefined;
    });
}
...
function htmlFragmentToText(fragment) {
    try {
        ...
        $('br').replaceWith('\n');
        $(BLOCK_BREAK_SELECTOR).each((_index, node) => {
            if (node.name !== 'br')
                $(node).append('\n');
        });
        ...
        return normalizeLyricsText(fragment
...
function collectHeuristicCandidates(html, add) {
    const patterns = [
...
function canonicalLineSet(text) {
    const lines = text
        .split('\n')
...
function addConsensus(winner, challenger) {
...
function mergeNearDuplicateCandidates(candidates) {
...
function finalConfidence(candidate) {
    let confidence = candidate.quality.confidence;
    if (candidate.signals.includes('multi-parser-consensus'))
        confidence += 0.05;
    if (candidate.signals.includes('cross-extractor-consensus'))
        confidence += 0.04;
    if (candidate.method === 'dom-semantic' || candidate.method === 'microdata')
        confidence += 0.05;
    if (candidate.method === 'hydration-state' || candidate.method === 'json-ld')
        confidence += 0.03;
    if (candidate.warnings.length)
        confidence -= Math.min(0.16, candidate.warnings.length * 0.05);
    return Number(clamp(confidence, 0, 0.995).toFixed(3));
}
export function extractLyricsAdvanced(rawHtml) {
    ...
    if (warnings.includes('anti-bot-page-detected') && best.quality.score < 72)
        return null;
    if (best.quality.score < 30 || best.quality.lineCount < 2)
        return null;
...
export function extractionEngineCapabilities() {
