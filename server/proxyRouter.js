...
import { clearCache, getCacheStats, getGospelSongLyrics, resetProviderHealth, searchGospelSongs } from './lyricsService.js';
import { OFFICIAL_PROXY_BASE_URL, getAdminToken, getProxyConfig, getSafeProxyConfig, resetProxyConfig, updateProxyConfig } from './proxyConfig.js';
import { LyricsRequestSchema, ProxyConfigUpdateSchema, RawProxyRequestSchema, SearchQuerySchema, zodIssueMessage } from './schemas.js';
...
function fallbackRateLimit(headers) {
    const config = getProxyConfig();
...
function truncateLogValue(value, max = 600) {
    if (value == null)
        return undefined;
    try {
...
function adminAuthorized(headers) {
    ...
    if (configuredToken)
        return Boolean(supplied) && safeSecretEqual(configuredToken, supplied);
    const config = getProxyConfig();
...
async function validateRawTarget(raw) {
    const config = getProxyConfig();
    const target = new URL(raw);
    if (!['http:', 'https:'].includes(target.protocol))
        throw new Error('Apenas HTTP/HTTPS é permitido');
...
function sanitizeRawHeaders(headers) {
    const sanitized = {
        'User-Agent': 'GospelLyricsProxy/2.6',
        Accept: '*/*',
    };
    for (const [key, value] of Object.entries(headers || {})) {
        if (!SAFE_RAW_HEADERS.has(key.toLowerCase()))
            continue;
        sanitized[key] = String(value).slice(0, 4000);
    }
    return sanitized;
}
async function fetchRawSafely(rawReq) {
    let target = await validateRawTarget(rawReq.targetUrl);
    for (const [key, value] of Object.entries(rawReq.queryParams || {}))
...
export function recordLog(log) {
    ...
    if (trafficLogs.length > 100)
        trafficLogs.length = 100;
    logger.info({
...
export async function handleApiRequest(url, method, headers, body) {
    ...
    if (normalizedMethod === 'OPTIONS')
        return { status: 204, headers: responseHeaders, body: null };
    ...
    try {
                ...
                body: {
                    ...
                    providerModes: {
                        ...
                        vagalume: config.providers.vagalume.enabled
                            ? (config.providers.vagalume.apiKey ? 'excerpt-index+artist-web+api-get' : 'excerpt-index+artist-web+glx-get')
                            : 'disabled',
                    },
                    scraperEngine: {
        ...
        if (pathname === '/api/proxy/lyrics/raw' && normalizedMethod === 'POST') {
            ...
            const rawReq = parsed.data;
            try {
                const { response: externalRes, target } = await fetchRawSafely(rawReq);
                const contentType = externalRes.headers.get('content-type') || '';
                const rawText = await externalRes.text();
                if (rawText.length > 1_500_000)
                    throw new Error('Resposta externa excede o limite permitido');
