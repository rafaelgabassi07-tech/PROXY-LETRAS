import { getAdminToken, getProxyConfig, OFFICIAL_PROXY_BASE_URL } from './proxyConfig.ts';
...
function corsHeaders(request: Request): Headers {
  ...
  if (config.enableCors) {
    if (config.allowedOrigins.includes('*')) headers.set('Access-Control-Allow-Origin', '*');
    else if (origin && config.allowedOrigins.includes(origin)) headers.set('Access-Control-Allow-Origin', origin);
  }
  return headers;
}

async function runtimeProbe(): Promise<{ ready: boolean; diagnostic?: string }> {
  try {
    ...
    const module = await import('./lyricsService.ts');
...
export async function healthFetch(request: Request): Promise<Response> {
  const headers = corsHeaders(request);
  if (request.method === 'OPTIONS') return new Response(null, { status: 204, headers });
  if (request.method !== 'GET') return Response.json({ status: 'error', code: 'METHOD_NOT_ALLOWED' }, { status: 405, headers });
...
export const vercelHealthHandler = { fetch: healthFetch } as const;
