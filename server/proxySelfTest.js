...
async function main() {
    ...
    try {
        ...
        globalThis.fetch = async (input) => {
            ...
            if (url === 'https://www.vagalume.com.br/ministerio-teste/') {
                return new Response('<html><body><a href="/ministerio-teste/cancao-vagalume.html">Canção Vagalume</a><a href="/ministerio-teste/outra-musica.html">Outra Música</a></body></html>', { status: 200, headers: { 'content-type': 'text/html; charset=utf-8' } });
            }
            if (url === 'https://www.vagalume.com.br/ministerio-teste/cancao-vagalume.html') {
                return new Response('<html><head><meta property="og:title" content="Canção Vagalume - Ministério Teste"></head><body><main><div class="lyrics">Verso um de louvor com texto suficientemente amplo para o extrator.<br>Graça e esperança em Jesus aparecem nesta canção de teste.<br>Refrão de adoração ao Senhor com conteúdo musical significativo.<br>Última linha da letra para confirmar a recuperação completa.</div></main></body></html>', { status: 200, headers: { 'content-type': 'text/html; charset=utf-8' } });
            }
        ...
        const remoteSearch = await handleApiRequest('/api/proxy/lyrics/search', 'POST', { 'x-forwarded-for': 'selftest-remote' }, { query: 'Canção da Graça', provider: 'letras_mus_br', limit: 5 });
        ...
        const cachedSearch = await handleApiRequest('/api/proxy/lyrics/search', 'POST', { 'x-forwarded-for': 'selftest-remote-cache' }, { query: 'Canção da Graça', provider: 'letras_mus_br', limit: 5 });
        assert.equal(cachedSearch.status, 200);
        assert.equal(cachedSearch.body.cached, true);
        assert.equal(remoteCalls, 1);
        const remoteGet = await handleApiRequest('/api/proxy/lyrics/get', 'POST', { 'x-forwarded-for': 'selftest-remote-get' }, { ...remoteHit, provider: remoteHit.source });
    ...
    try {
        ...
        const customSearch = await handleApiRequest('/api/proxy/lyrics/search', 'POST', { 'x-forwarded-for': 'selftest-custom' }, { query: 'Canção Custom', provider: 'custom', limit: 3 });
        assert.equal(customSearch.status, 200);
        const customHit = customSearch.body.data[0];
        assert.equal(customHit.source, 'custom_api');
        const customGet = await handleApiRequest('/api/proxy/lyrics/get', 'POST', { 'x-forwarded-for': 'selftest-custom-get' }, { ...customHit, provider: customHit.source });
