        ...
        if (allow('vagalume') && config.providers.vagalume.enabled && providerAvailable('vagalume')) {
            jobs.push({
                ...
                run: async () => {
                    ...
                    try {
                        const indexed = await searchVagalumeExcerpt(config.providers.vagalume.baseUrl, config.providers.vagalume.webBaseUrl, query, limit, config.providers.vagalume.timeoutMs);
                        if (indexed.length)
                            return indexed;
                    }
                    catch { /* fallback HTML abaixo */ }
                    return searchVagalumeWeb(config.providers.vagalume.webBaseUrl, titleQuery || query, normalizedParams.artist, limit, config.providers.vagalume.timeoutMs);
                },
            });
        }
