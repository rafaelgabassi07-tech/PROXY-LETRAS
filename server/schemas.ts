...
const optionalTrimmed = (max: number) => trimmed(max).optional().default('');
...
export const LyricsRequestSchema = z.object({
  ...
  (value: { id: string; artist: string; title: string; sourceUrl: string }) => Boolean(value.id || value.artist || value.title || value.sourceUrl),
  { message: 'Informe id, título/artista ou sourceUrl.' },
);
